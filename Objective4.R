...
#creation of scree plot and cumaltive curve
pca.result.classify$rotation
eigenvals<-(pc.result.classify$sdev)^2
plot(1:19,eigenvals/sum(eigenvals),type="l",main="Scree Plot",ylab="Prop. Var. Explained")
cumulative.prop<-cumsum(eigenvals/sum(eigenvals))
plot(1:19,cumulative.prop,type="l",main="Cumulative proportion",ylim=c(0,1))
...
ggplot(data = pca.result.classify.score, aes(x = PC5, y = PC6)) +
  geom_point(aes(col=GOOD1), size=1)+
  ggtitle("PCA of GOOD PC5vsPC6")

ggplot(data = pca.result.classify.score, aes(x = PC6, y = PC7)) +
  geom_point(aes(col=GOOD1), size=1)+
  ggtitle("PCA of GOOD PC6vsPC7")

ggplot(data = pca.result.classify.score, aes(x = PC7, y = PC8)) +
  geom_point(aes(col=GOOD1), size=1)+
  ggtitle("PCA of GOOD PC7vsPC8")

ggplot(data = pca.result.classify.score, aes(x = PC8, y = PC9)) +
  geom_point(aes(col=GOOD1), size=1)+
  ggtitle("PCA of GOOD PC8vsPC9")


ggplot(data = pca.result.classify.score, aes(x = PC9, y = PC10)) +
  geom_point(aes(col=GOOD1), size=1)+
  ggtitle("PCA of GOOD PC9vsPC10")


pca.result.classify$rotation[,1]

# LDA analysis

mylda <- lda(GOOD ~ ., data = df.train[,-3])
pred<-predict(mylda,newdata=df.testa)$class
x<-table(pred,df.testa$GOOD) # Creating a confusion matrix
ME<-(x[2,1]+x[1,2])/1000 #Missclassification Error
ME
1-ME #Calculating overall accuracy
confusionMatrix(table(pred,df.testa$GOOD))
